...
def main():
    ...
    with mock_api.api(job_name_prefix='boolean_int_params_') as api:
        api.mock_job('job-1', exec_time=0.5, max_fails=0, expect_invocations=1, expect_order=1, job_xml_template=template, params=(('b1', False, 'boolean'), ('b2', True, 'boolean')))
        api.mock_job('job-2', exec_time=0.5, max_fails=0, expect_invocations=1, expect_order=2, job_xml_template=template, params=(('i1', 1, 'boolean'), ('i2', 2, 'boolean')))
