...
def main():
    ...
    api = mock_api.api(job_name_prefix='tst_')
    if mock_api.is_mocked:
        with api:
            api.mock_job('job-1', exec_time=3, max_fails=0, expect_invocations=1, expect_order=1)
            api.mock_job('job-2', exec_time=3, max_fails=0, expect_invocations=1, expect_order=2)
            api.mock_job('job-3', exec_time=3, max_fails=0, expect_invocations=1, expect_order=3)
            api.mock_job('job-4', exec_time=3, max_fails=0, expect_invocations=1, expect_order=3)
            api.mock_job('job-5', exec_time=3, max_fails=0, expect_invocations=1, expect_order=3)
            api.mock_job('job-6', exec_time=3, max_fails=0, expect_invocations=1, expect_order=4)
            api.mock_job('job-7', exec_time=3, max_fails=0, expect_invocations=1, expect_order=5)
